  ...
  if (schema[import_typebox.Kind] === fromSymbol) {
    ...
    const composeProperties = (v) => {
      ...
      if (properties2 && v.type === "string" && v.format === "ObjectString" && v.default === "{}") {
        transform = import_type_system.t.ObjectString(properties2, rest);
        value.default = JSON.stringify(
          import_value.Value.Create(import_type_system.t.Object(properties2))
        );
        value.properties = properties2;
      }
      if (items && v.type === "string" && v.format === "ArrayString" && v.default === "[]") {
        transform = import_type_system.t.ArrayString(items, rest);
        value.default = JSON.stringify(import_value.Value.Create(import_type_system.t.Array(items)));
        value.items = items;
      }
      return value;
    };
...
const createCleaner = (schema) => (value) => {
  if (typeof value === "object")
    try {
      return import_value.Value.Clean(schema, structuredClone(value));
    } catch {
      try {
...
const getSchemaValidator = (s, {
  ...
  const compiled = import_compiler.TypeCompiler.Compile(schema, Object.values(models));
