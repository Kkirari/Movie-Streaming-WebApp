...
`;let index=0;for(let{key,isArray,isObject,isNestedObjectArray,anyOf}of destructured){let init2=(index===0?"let ":"")+`memory=url.indexOf('&${key}=')
let a${index}
...
if(memory===-1) break
}`;else if(isObject)fnLiteral+=init2+`if(memory!==-1){const start=memory+${key.length+2}
memory=url.indexOf('&',start)
...
`;if(hasProperty("default",validator.headers))for(let[key,value]of Object.entries(Value.Default(validator.headers.schema,{}))){let parsed=typeof value==="object"?JSON.stringify(value):typeof value==="string"?`'${value}'`:value;if(parsed!==void 0)fnLiteral+=`c.headers['${key}']??=${parsed}
...
`;if(hasProperty("default",validator.query))for(let[key,value]of Object.entries(Value.Default(validator.query.schema,{}))){let parsed=typeof value==="object"?JSON.stringify(value):typeof value==="string"?`'${value}'`:value;if(parsed!==void 0)fnLiteral+=`if(c.query['${key}']===undefined)c.query['${key}']=${parsed}
...
`;if(hasProperty("default",validator.body)){let value=Value.Default(validator.body.schema,validator.body.schema.type==="object"?{}:void 0),parsed=typeof value==="object"?JSON.stringify(value):typeof value==="string"?`'${value}'`:value;if(fnLiteral+=`if(validator.body.Check(c.body)===false){if(typeof c.body==='object')c.body=Object.assign(${parsed},c.body)
else c.body=${parsed}
...
`,hasProperty("default",cookieValidator))for(let[key,value]of Object.entries(Value.Default(cookieValidator.schema,{})))fnLiteral+=`cookieValue['${key}'] = ${typeof value==="object"?JSON.stringify(value):value}
...
`;for(let i2=0;i2<hooks.mapResponse.length;i2++){let mapResponse=hooks.mapResponse[i2],endUnit2=mapResponseReporter.resolveChild(mapResponse.fn.name);fnLiteral+=`if(mr===undefined){mr=${isAsyncName(mapResponse)?"await ":""}e.mapResponse[${i2}](c)
...
c.response=r
`;for(let i=0;i<hooks.mapResponse.length;i++){let mapResponse=hooks.mapResponse[i],endUnit=mapResponseReporter.resolveChild(mapResponse.fn.name);fnLiteral+=`
...
`;for(let i=0;i<hooks.mapResponse.length;i++){let mapResponse=hooks.mapResponse[i],endUnit=mapResponseReporter.resolveChild(mapResponse.fn.name);fnLiteral+=`if(mr===undefined){mr=${isAsyncName(mapResponse)?"await ":""}e.mapResponse[${i}](c)
...
`;endUnit();let mapResponseReporter=report("mapResponse",{total:hooks.mapResponse?.length});if(hooks.mapResponse?.length)for(let i2=0;i2<hooks.mapResponse.length;i2++){let mapResponse=hooks.mapResponse[i2],endUnit2=mapResponseReporter.resolveChild(mapResponse.fn.name);fnLiteral+=`c.response=er
...
`;let mapResponseReporter2=report("mapResponse",{total:hooks.mapResponse?.length,name:"context"});if(hooks.mapResponse?.length)for(let i2=0;i2<hooks.mapResponse.length;i2++){let mapResponse=hooks.mapResponse[i2],endUnit=mapResponseReporter2.resolveChild(mapResponse.fn.name);fnLiteral+=`context.response=_r_r=${isAsyncName(mapResponse)?"await ":""}onMapResponse[${i2}](context)
...
`},formData(isOptional2){let fnLiteral=`
c.body={}
`;if(isOptional2)fnLiteral+="let form;try{form=await c.request.formData()}catch{}";else fnLiteral+=`const form=await c.request.formData()
...
//# debugId=B65EEADBB055FB0264756E2164756E21
